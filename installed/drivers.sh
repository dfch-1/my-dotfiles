...
echo
echo "$(mng detecting_cpu)"
CPU_VENDOR=$(grep -m1 "vendor_id" /proc/cpuinfo | awk '{print $3}')

case "$CPU_VENDOR" in
    GenuineIntel)
        echo "$(mng installing_intel_ucode)"
        sudo pacman -S --needed --noconfirm intel-ucode
        ;;
    AuthenticAMD)
        echo "$(mng installing_amd_ucode)"
